...
final class XcodeProjectGenerator {
  ...
  func resolveExternalReferencePath(_ path: String, xcodeProject: PBXProject) -> String? {
    ...
    guard let relativeWorkspacePath = xcodeProject.mainGroup.path else {
        return nil
    }
    let relativeRoot = relativeWorkspacePath + "/" + PBXTargetGenerator.TulsiWorkspacePath
  ...
  private func patchExternalRepositoryReferences(_ xcodeProject: PBXProject) {
    ...
    for child in externalChildren {
      guard let resolvedPath = resolveExternalReferencePath("external/\(child.path!)", xcodeProject: xcodeProject) else {
