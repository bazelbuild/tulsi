...
def _collect_dsym_bundles_from_deps(target):
  """Returns a list of bundle names that contain dSYM symbols.

  In order to get all dSYM bundles generated by a target, including those
  generated by its dependencies, this method will return all bundles from the
  deps that contain the AppleDebugOutputs provider, which is only generated
  when debug symbols are present.

  Args:
    target: A `Target` instance.

  Returns:
    A `list` of `string`s representing bundle names.
  """
  dsyms = []

  # TODO(b/37912213): Migrate to the new-style providers.
  if (hasattr(target, 'apple_bundle') and
      # Check that the rules_apple code contains bundle_extension attribute,
      # for backwards compatibility.
      hasattr(target.apple_bundle, 'bundle_extension') and
      # Check that the target has debug outputs.
      apple_common.AppleDebugOutputs in target):
    apple_bundle = target.apple_bundle
    dsyms.append(apple_bundle.bundle_name + apple_bundle.bundle_extension)
...
def _tulsi_outputs_aspect(target, ctx):
  ...
  for attr_name in _TULSI_COMPILE_DEPS:
    deps = _getattr_as_list(rule_attr, attr_name)
    for dep in deps:
      dsym_bundle_names.extend(_collect_dsym_bundles_from_deps(dep))
  ...
  info = _struct_omitting_none(
      ...
      bundle_name=bundle_name,
      dsym_bundle_names=dsym_bundle_names)
