...
final class PBXTargetGenerator: PBXTargetGeneratorProtocol {
    ...
    func generateIndexerTargetGraphForRuleEntry(ruleEntry: RuleEntry) -> (Set<String>,
      ...
      if let ruleIncludes = ruleEntry.attributes[.includes] as? [String] {
        ...
        ruleIncludes.forEach() {
          ...
          includes.addObject("$(\(PBXTargetGenerator.BazelWorkspaceSymlinkVarName))/\(bazelBinPath)/\(packageQualifiedPath)")
          includes.addObject("$(\(PBXTargetGenerator.BazelWorkspaceSymlinkVarName))/\(bazelGenfilesPath)/\(packageQualifiedPath)")
        }
      }
      ...
      if !buildPhase.files.isEmpty {
        ...
        for module in ruleEntry.swiftTransitiveModules {
          ...
          swiftIncludePaths.addObject("$(\(PBXTargetGenerator.BazelWorkspaceSymlinkVarName))/\(includePath)")
        }
  ...
  func generateTopLevelBuildConfigurations(projectSDKROOT projectSDKROOT: String?) {
    ...
    let searchPaths = ["$(\(PBXTargetGenerator.BazelWorkspaceSymlinkVarName))",
                       "$(\(PBXTargetGenerator.BazelWorkspaceSymlinkVarName))/\(bazelBinPath)",
                       "$(\(PBXTargetGenerator.BazelWorkspaceSymlinkVarName))/\(bazelGenfilesPath)",
    ]
