...
class BazelBuildBridge(object):
  ...
  def _UpdateLLDBInit(self):
    ...
    if not os.path.isfile(lldbinit_path):
      raw_lldbinit_path = os.path.expanduser('~/.lldbinit')
      if os.path.isfile(raw_lldbinit_path):
        shutil.copy(raw_lldbinit_path, lldbinit_path)
  ...
  def _ClearLLDBInit(self):
    """Updates ~/.lldbinit-Xcode to remove Tulsi-generated info."""
    lldbinit_path = os.path.expanduser('~/.lldbinit-Xcode')
    content = self._ExtractLLDBInitContent(lldbinit_path)
    if not content:
